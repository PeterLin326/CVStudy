...
class DiTBlock(nn.Module):
    ...
    def forward(self,x,cond): #x:(batchm seq_len, emb_size), cond:(batch, emb_size)
        ...
        k=self.wk(y)
        v=self.wv(y)
        q=q.view(q.size(0),q.size(1),self.nhead,self.emb_size).permute(0,2,1,3) #(B,nhead,seq_len, emb_size)
        k=k.view(k.size(0),k.size(1),self.nhead,self.emb_size).permute(0,2,3,1) #(B,nhead,emb_size,seq_len)
        v=v.view(v.size(0),v.size(1),self.nhead,self.emb_size).permute(0,2,1,3) #(B,nhead,seq_len,emb_size)
        
        attn=q@k/math.sqrt(q.size(2)) #(B,nhead,seq_len,seq_len)
        attn=torch.softmax(attn,dim=-1) #(B,nhead,seq_len,seq_len)
        y=attn@v
        y=y.permute(0,2,1,3)
